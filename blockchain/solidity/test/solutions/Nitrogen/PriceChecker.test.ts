...
describe('Test Price Checker for TokenVault', () => {
    it('Check price for ERC-20 (not extension)', async () => {
        const { usdcVault, USDC, user0, poolPriceChecker } = await loadFixture(
        ...
        await poolPriceChecker.changePriceDeviationBps(USDC, 0);
        ...
        await poolPriceChecker.changePriceDeviationBps(USDC, (5 * 10_000) / 100); // 5%
        await usdcVault
            .connect(user0)
            ['deposit(uint256,address,address)'](depositValue, user0, user0);

        await poolPriceChecker.setPriceFeed(USDC, ethers.ZeroAddress, 0, 0);
        await usdcVault
            .connect(user0)
            ['deposit(uint256,address,address)'](depositValue, user0, user0);
    });
    ...
    it('Check price errors', async () => {
        ...
        await poolPriceChecker.setPriceFeed(USDC, usdcFeed, (5 * 10_000) / 100, days(1));
        await vaultPriceChecker.setPriceFeed(USDC, usdcFeed, (5 * 10_000) / 100, days(1));
    ...
    it('Check price for sUSDe using sUSDe', async () => {
        const { susdeVault, sUSDe, user0, poolPriceChecker } = await loadFixture(
            deployNitrogenWithTokenVault,
        );
        ...
        // sUSDe / USD
        await poolPriceChecker.changePriceDeviationBps(sUSDe, 0);
        ...
        await poolPriceChecker.changePriceDeviationBps(sUSDe, (5 * 10_000) / 100); // 5%
        await susdeVault
            .connect(user0)
            ['deposit(uint256,address,address)'](depositValue, user0, user0);
    });
